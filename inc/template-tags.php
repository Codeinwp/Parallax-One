...
function the_post_navigation() {
	...
	<nav class="navigation post-navigation" role="navigation">
		<h2 class="screen-reader-text"><?php _e( 'Post navigation', 'parallax-one' ); ?></h2>
		<div class="nav-links">
			<?php
...
function parallax_one_entry_footer() {
	...
	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( __( 'Leave a comment', 'parallax-one' ), __( '1 Comment', 'parallax-one' ), __( '% Comments', 'parallax-one' ) );
		echo '</span>';
	}

	edit_post_link( __( 'Edit', 'parallax-one' ), '<span class="edit-link">', '</span>' );
}
endif;
...
function the_archive_title( $before = '', $after = '' ) {
	if ( is_category() ) {
		$title = sprintf( __( 'Category: %s', 'parallax-one' ), single_cat_title( '', false ) );
	} elseif ( is_tag() ) {
		$title = sprintf( __( 'Tag: %s', 'parallax-one' ), single_tag_title( '', false ) );
	} elseif ( is_author() ) {
		$title = sprintf( __( 'Author: %s', 'parallax-one' ), '<span class="vcard">' . get_the_author() . '</span>' );
	} elseif ( is_year() ) {
		$title = sprintf( __( 'Year: %s', 'parallax-one' ), get_the_date( _x( 'Y', 'yearly archives date format', 'parallax-one' ) ) );
	} elseif ( is_month() ) {
		$title = sprintf( __( 'Month: %s', 'parallax-one' ), get_the_date( _x( 'F Y', 'monthly archives date format', 'parallax-one' ) ) );
	} elseif ( is_day() ) {
		$title = sprintf( __( 'Day: %s', 'parallax-one' ), get_the_date( _x( 'F j, Y', 'daily archives date format', 'parallax-one' ) ) );
		...
		$title = sprintf( __( 'Archives: %s', 'parallax-one' ), post_type_archive_title( '', false ) );
	} elseif ( is_tax() ) {
		$tax = get_taxonomy( get_queried_object()->taxonomy );
		/* translators: 1: Taxonomy singular name, 2: Current taxonomy term */
		$title = sprintf( __( '%1$s: %2$s', 'parallax-one' ), $tax->labels->singular_name, single_term_title( '', false ) );
	} else {
		$title = __( 'Archives', 'parallax-one' );
	}
